...
const ProfileScreen: React.FC = () => {
    const { authData, logout } = useAuth(); // Access authData from the context
    const [user, setUser] = useState<any>(null); // State to hold user data

    useEffect(() => {
        const fetchUserData = async () => {
            try {
                const response = await axios.get(`${process.env.EXPO_PUBLIC_BASE_URL}/api/v1/user/get/${authData.id}`); // Use the id from authData
